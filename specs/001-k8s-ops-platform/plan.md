...
**Branch**: `001-k8s-ops-platform` | **Date**: 2026-04-09 | **Spec**: [/mnt/e/code/kbManage/specs/001-k8s-ops-platform/spec.md](/mnt/e/code/kbManage/specs/001-k8s-ops-platform/spec.md)
...
构建一个前后端分离的 Web 化 Kubernetes 多集群管理平台，首期提供统一集群接入与资源总览、平台级 RBAC 与工作空间/项目双层权限、受控运维操作执行、审计追踪与导出。技术上采用 React + TypeScript + Ant Design + Vite 的前端栈，Go + Gin + client-go + GORM 的后端栈，MySQL 负责平台元数据与审计持久化，Redis 负责会话、缓存与异步任务协同。系统按模块化单体实现，通过“平台控制面 + 集群访问/同步模块 + 操作执行模块 + 审计模块”组织代码，暂不纳入 CI/CD、可观测和交付部署设计。
...
**Git Workflow**: 当前设计分支为 `001-k8s-ops-platform`，远程为 `git@github.com:baihua19941101/kbManage.git`；实施阶段需继续在专用功能分支开发，完成后推送远程并提交中文 PR，合并前必须获得用户明确同意  
**Performance Goals**: 支持 20 个已接入集群与至少 10,000 个受管资源；资源筛选 1 分钟内完成；95% 常见运维操作 2 分钟内给出明确结果；90% 的 90 天审计查询 30 秒内返回  
**Constraints**: 暂不接入 SSO/Keycloak，仅支持用户名密码登录；权限模型固定为平台级 RBAC + 工作空间/项目双层权限；CI/CD、可观测和交付部署不纳入本轮设计；所有设计与交付文档必须中文；子代理如被使用必须固定为 `gpt-5.3-codex`  
...
- `PASS`: 当前工作分支为 `001-k8s-ops-platform`，满足“禁止在 `master/main` 直接开发”的要求。
- `PASS`: 已记录实施前的 MySQL 备份命令、产物路径和恢复校验步骤；设计阶段不实际修改数据库，因此本门禁已具备可执行方案。
