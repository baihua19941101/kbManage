...
type marketplaceIntegrationCtx struct {
	Router *gin.Engine
	DB     *gorm.DB
	Token  string
...
func newMarketplaceIntegrationCtx(t *testing.T, roleKey string) *marketplaceIntegrationCtx {
	t.Helper()
	app := testutil.NewApp(t)
	user := testutil.SeedUser(t, app.DB, testutil.SeedUserInput{
		Username: "marketplace-integration-" + strings.ReplaceAll(roleKey, "_", "-"),
		Password: "Integration@123",
	})
	access := testutil.SeedObservabilityAccess(t, app.DB, user.User.ID, "marketplace-integration", roleKey)
	...
	return &marketplaceIntegrationCtx{
		Router: engine,
		DB:     app.DB,
		Token:  token,
		Access: access,
...
func performMarketplaceIntegrationRequest(t *testing.T, h http.Handler, token, method, path, body string) *httptest.ResponseRecorder {
	t.Helper()
