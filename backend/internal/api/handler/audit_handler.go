...
func (h *AuditHandler) ListClusterLifecycleEvents(c *gin.Context) {
	startAt, err := parseOptionalRFC3339(firstNonEmptyQuery(c, "startAt", "timeFrom"), "startAt")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	items, err := h.svc.QueryClusterLifecycleEvents(c.Request.Context(), auditSvc.QueryEventsRequest{
		StartAt:     startAt,
		EndAt:       endAt,
		ClusterID:   clusterID,
		WorkspaceID: workspaceID,
...
func (h *AuditHandler) ListBackupRestoreEvents(c *gin.Context) {
	startAt, err := parseOptionalRFC3339(firstNonEmptyQuery(c, "startAt", "timeFrom"), "startAt")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	items, err := h.svc.QueryBackupRestoreEvents(c.Request.Context(), auditSvc.QueryEventsRequest{
		StartAt:     startAt,
		EndAt:       endAt,
		ClusterID:   clusterID,
		WorkspaceID: workspaceID,
...
func (h *AuditHandler) ListIdentityTenancyEvents(c *gin.Context) {
	startAt, err := parseOptionalRFC3339(firstNonEmptyQuery(c, "startAt", "timeFrom"), "startAt")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	items, err := h.svc.QueryIdentityTenancyEvents(c.Request.Context(), auditSvc.QueryEventsRequest{
		StartAt:     startAt,
		EndAt:       endAt,
		ClusterID:   clusterID,
		WorkspaceID: workspaceID,
...
func (h *AuditHandler) ListEnterpriseEvents(c *gin.Context) {
	startAt, err := parseOptionalRFC3339(firstNonEmptyQuery(c, "startAt", "timeFrom"), "startAt")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	items, err := h.svc.QueryEnterpriseEvents(c.Request.Context(), auditSvc.QueryEventsRequest{
		StartAt: startAt, EndAt: endAt, ClusterID: clusterID, WorkspaceID: workspaceID, ProjectID: projectID,
		Action: strings.TrimSpace(c.Query("action")), Outcome: strings.TrimSpace(c.Query("outcome")),
		Result: strings.TrimSpace(c.Query("result")), Resource: strings.TrimSpace(c.Query("resource")), Limit: 100,
		ViewerID: c.GetUint64(middleware.UserIDKey),
	})
	if err != nil {
		writeAuditError(c, err)
...
func (h *AuditHandler) ListPlatformMarketplaceEvents(c *gin.Context) {
	startAt, err := parseOptionalRFC3339(firstNonEmptyQuery(c, "startAt", "timeFrom"), "startAt")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	items, err := h.svc.QueryPlatformMarketplaceEvents(c.Request.Context(), auditSvc.QueryEventsRequest{
		StartAt:     startAt,
		EndAt:       endAt,
		ClusterID:   clusterID,
		WorkspaceID: workspaceID,
